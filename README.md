...
- ショップ割引券：市場ガチャで5%OFF（8%）・10%OFF（3%）が自動付与される。利用時は総合お問い合わせで教団市場チケットを切り、従業員に割引後の支払額を確認してから、その金額をショップパネルで送金する。100万krm以上の商品には利用できない
- 教祖雑談無料・カスタムロール一週間・教祖遠隔・異端通行券3日分：利用時は[総合お問い合わせ](https://discord.com/channels/1520329128883126392/1520368587255189545)で教団市場チケットを切り、当選メッセージを提示する。教祖遠隔・異端通行券3日分は教祖さんをメンションする
...
`1526626039844049037` のTCで技術統括が `/panel` を実行すると、おみくじパネルを設置できます。口座を持つメンバーは無料で、日本時間で1日1回だけ引けます。鯖主・技術統括を含め、ロールによる回数制限の例外はありません。サブアカウントは引けません。

- 小吉: 1,000krm（34.5%）
- 中吉: 2,000krm（59%）
- 大吉: 5,000krm（5%）
- 凶: -3,000krm（1%）。残高が足りない場合は0krmまでに留めます。
- 超大吉: 50,000krm（0.5%）
