...
運営（鯖主・管理者・技術統括・イベント統括・イベントスタッフ）は、以下の順で運用します。

1. `/roulette open stage:第1部` で受付を開始する。
2. 必要なら `/roulette close` で明示的に締め切る。`/result` は受付中でも自動的に締め切って精算する。
3. ディーラーが `/result number:0〜36` を実行する。結果と当選・配当は実行チャンネルへ投稿され、通貨残高と取引履歴が同時に更新される。
4. 次のラウンドは、前の結果を確定した後に再び `/roulette open` を実行する。第1部・第2部を2回ずつ行う場合も同様です。
5. 終了時に `/roulette bonus` を一度実行すると、そのイベントで一度でも確定ベットした参加者へ30,000通貨を配布する。二重配布はされません。
