...
`src/service` は口座・通貨・ホテル・遊戯・市場・評価などの機能別フォルダーに分けています。
各機能の処理とパネルは同じ場所に置きます。配置先と追加時のルールは
[サービスの配置](src/service/README.md)を参照してください。
...
システム支配人（`ROLE_IDS.GIJUTU_LEADER`）は全コマンド・パネルの操作権限を持ちます。新しい操作権限チェックには `src/util/operatorPermission.ts` の `hasOperatorRole` を使い、システム支配人を許可します。`hasRole` は対象者の属性・料金・所有ロールを調べる関数として維持します。
