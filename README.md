...
支払先はうゆSub（`1400304116152139837`）に固定し、送金先の選択はありません。支払い後、スレッド `1549981595640008866` に購入者・金額・商品・適用ロールをEmbedで記録します。残高の更新と取引履歴の保存は同じトランザクションで行い、同じ確認画面からの重複送金を防止します。ログ投稿だけに失敗した場合は、送金済みであることと再度支払わない旨を本人に案内します。再設置は支払い場で `/panel` を実行してください。
