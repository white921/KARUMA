...
1. `/賭け開始 number:1〜3` で、開催する部を指定して受付を開始する。
2. 必要なら `/賭け終了` で明示的に締め切る。`/結果` は受付中でも自動的に締め切って精算する。
3. ディーラーが `/結果 color:色 number:0〜36` を実行する。色は数値との不一致を防ぐ確認項目で、赤・黒の表記ゆれに対応する。`0` のときだけ `緑` を選ぶ。結果と当選・配当は実行チャンネルへ投稿され、通貨残高と取引履歴が同時に更新される。
