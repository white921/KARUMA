...
export function createGachaCoinPanelPayload() {
  return {
    embeds: [new EmbedBuilder().setTitle(GACHA_COIN_PANEL_TITLE).setColor(COLOR.LIGFT_PINK)
      .setDescription("ガチャコインをアイテムと交換できます。交換するチケットを選び、内容を確認して確定してください。")
      .addFields(
    ...
    components: [
      new ActionRowBuilder<ButtonBuilder>().addComponents(...GACHA_COIN_REWARDS.map(r => new ButtonBuilder()
        .setCustomId(`${GACHA_COIN_PREFIX}:select:${r.key}`).setLabel(`${r.label}（${r.cost}枚）`).setStyle(ButtonStyle.Success))),
