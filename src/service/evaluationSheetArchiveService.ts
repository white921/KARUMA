...
<style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;max-width:900px;margin:32px auto;padding:0 16px;background:#f8f9fb;color:#202124}article{background:#fff;border:1px solid #e2e5e9;border-radius:8px;margin:12px 0;padding:14px}header{display:flex;gap:12px;align-items:baseline}.time{color:#6b7280;font-size:12px}.content{white-space:pre-wrap;overflow-wrap:anywhere}.attachments{margin:8px 0 0;padding-left:20px}</style>
</head><body><h1>${this.escapeHtml(threadName)}</h1><p>対象ユーザーID: ${this.escapeHtml(userId)}</p><p>保存日時: ${new Date().toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}</p>${entries || "<p>メッセージはありません。</p>"}</body></html>`;
  }
  ...
  private static createMessageHtml(message: Message): string {
    const author = this.escapeHtml(message.author.tag);
    ...
    return `<article><header><strong>${author}</strong><span class="time">${time}</span></header><div class="content">${this.escapeHtml(contentParts.join("\n\n") || "（本文なし）")}</div>${attachments ? `<ul class="attachments">${attachments}</ul>` : ""}</article>`;
  }
