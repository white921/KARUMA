...
export const data = new SlashCommandBuilder()
  .setName(COMMAND_NAMES.GACHA_COIN_GRANT).setDescription("ユーザーのガチャコインを付与します").setDMPermission(false)
  .addUserOption(o => o.setName("ユーザー").setDescription("対象ユーザー").setRequired(true))
  .addIntegerOption(o => o.setName("枚数").setDescription("付与するコイン枚数").setMinValue(1).setMaxValue(GACHA_COIN_MAX).setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  await interaction.editReply({ content: `✅ <@${user.id}> のガチャコインを ${amount}枚付与しました。残高: **${balance}枚**`, allowedMentions: { parse: [] } });
}
